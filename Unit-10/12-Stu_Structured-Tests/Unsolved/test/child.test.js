...
describe("Child", () => {
    describe("nameAge", () => {
        it("Should throw an error if name input is empty string"), () => {
            const name = "suji";
            const age = "30";
    
            const outputName = "";
    
            const result = new Child(name, age);
    
            expected(result).toEqual(outputName, age);
        };

        it("should throw an error if age is empty integer"), () => {
            const result = new Child("Suji", "");

            expect(result.name).toEqual("suji");
